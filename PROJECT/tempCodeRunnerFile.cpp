...
// Struct for priority queue in Dijkstra
struct Node {
    int vertex;
    double dist;
    vector<int> path;
        ...
        for (int v = 0; v < n; v++) {
            if (graph[u][v].distance > 0) {
                ...
                if (useDistance) {
                    ...
                    weight *= trafficMultiplier; // apply traffic multiplier
                }
...
int main() {
    if (!loadPlaces("places.json")) return 1;
    int n = (int)placeNames.size();

    if (!loadEdges("edges.json", n)) return 1;

    specialDay = isSpecialDayFromFile("events.json");

    cout << "What time of day is it? (morning / noon / evening / night): ";
    string timeOfDay;
    cin >> timeOfDay;
    ...
    if (timeOfDay == "morning" || timeOfDay == "evening") {
        ...
        cout << "Invalid time of day. Defaulting to no extra traffic.\n";
    ...
    cout << "Calculating shortest paths by time (with traffic)...\n";
